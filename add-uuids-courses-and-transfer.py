    ...
    for line in contents:
        ...
        if start_applying and header_index < len(header_info):
            if (
                ...
                and header_info[header_index]["type"] == "part"
            ) or (
                line.startswith("## ")
                and header_info[header_index]["type"] == "chapter"
            ):
                updated_contents.append(
                    f"<{header_info[header_index]['type']}Id>{header_info[header_index]['uuid']}</{header_info[header_index]['type']}Id>\n"
                )
                header_index += 1
...
# Main execution path
path = "../sovereign-university-data/courses/hist201/"
