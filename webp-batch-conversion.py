    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        for file in files:
            if file.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                )  # Debugging: Show which file is being processed
                ...
                sys.stdout.write(
                    f"\rConverted {num_converted_files}/{total_files} files. Last: {file} -> {os.path.basename(webp_path)} | Reduction: {reduction_percentage:.2f}%"
                )
                sys.stdout.flush()
        ...
        for file in files:
            if file.lower().endswith(".md"):
                ...
                updated_content = re.sub(
                    r"\.(png|jpg|jpeg)(?!\.webp)", ".webp", content, flags=re.IGNORECASE
                )
...
if __name__ == "__main__":
    root_dir = "../planB-premium-content/courses/btc105/"
    convert_to_webp_and_remove_metadata(
        root_dir
    )  # Converts non-WebP files and strips metadata
